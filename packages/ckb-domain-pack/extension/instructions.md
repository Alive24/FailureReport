...
Use `ckb__recommend_log` when a narrow, privacy-bounded structured log would reduce diagnostic ambiguity. The prepared Codex delegation begins with `$failure-report-ckb-debugging`, which loads the unique CKB native skill from the Root-provisioned worktree symlink; do not attempt to use an Eve skill-loader tool.
