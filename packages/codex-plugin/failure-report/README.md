...
The mapping is exact and path-free at the public boundary: Issue content can select only `owner/repository`, and it cannot add or override a checkout. The supervisor uses `pnpm --filter @Alive24/FailureReport start -- --target-workspace <trusted-checkout>` internally. Eve resolves that verified checkout's current commit to a full immutable SHA, exposes only the repository/SHA/runtime-instance proof through the authenticated `/failure-report/v1/runtime` route, and makes the same private target identity available to Root's first Issue read. The adapter calls Root only after health and the complete binding match, so an Issue-only request never needs a reporter-supplied SHA. Concurrent requests single-flight through private locks. State and redacted startup logs default beneath the operating user's state directory; `FAILURE_REPORT_RUNTIME_STATE_ROOT` may select an operator-managed private volume.

The producer ID and immutable GitHub actor ID are deployment-owned provenance for managed workpad comments. The installed plugin forwards those explicit settings to Eve; it never infers the actor from a mutable login. `FAILURE_REPORT_GITHUB_WORKPAD_PRODUCERS` may register trusted historical producer IDs, and `FAILURE_REPORT_HANDOFF_DELIVERY_POLICY` may supply the repository-owned optional delivery policy.
