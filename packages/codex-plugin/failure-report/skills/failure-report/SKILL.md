...
Runtime provisioning is operator-owned and private. Never ask a reporter for a checkout, worktree, branch, port, Eve command, process detail, or runtime log. The adapter verifies the configured remote runtime or resolves a managed-local checkout from a trusted operator mapping before Root runs. If it returns a `FailureReport runtime pending [...]` result, preserve the Issue and `request_id`, show only the sanitized operator recovery guidance to an authorized operator, and retry the identical request after private provisioning is repaired. Never fall back to ordinary local diagnosis or place a host path in public content.
