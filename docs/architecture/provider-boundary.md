...
Each configured repository binds a Markdown template to one GitHub Project v2. `begin_failure_report` moves an accepted intake to the exact configured `Failure Report` option. That option is Project configuration, not a Shea Symphony workflow state: Shea does not claim it. The intake transition permits only unset, `Failure Report`, `Need Human Input`, `Backlog`, or `Todo`; an active downstream state fails closed rather than being regressed.

For a fully Ready report, delivery first performs the revision-bound render. It canonicalizes the configured template path, requires it to remain beneath the Eve application root, validates known and required variables, and renders only the human-facing view. FailureReport then appends the complete `failure-report/implementation-handoff/v1` and deterministic delivery intent in a folded JSON block. The structured contract and its identity never depend on the template.

The GitHub gateway creates a new comment and never edits an existing handoff comment. A deterministic delivery marker allows an identical process-loss retry to reuse its own verified comment; duplicate, foreign-actor, or content-conflicting markers require input. After comment readback, the Project adapter adds the Issue item if necessary, resolves one exact configured status field and option, changes only to `Backlog` or `Todo`, and requires readback. `failure-report/handoff-delivery/v1` acknowledges that boundary. It makes no claim about downstream implementation or review: `Backlog` waits for manual promotion and `Todo` is merely eligible for Shea Symphony to claim.
...
One revision that cannot fit an empty comment is encoded as ordered, content-addressed provisional chunk comments. Those comments are folded and explicitly non-authoritative. A final manifest is the only visibility boundary: it binds the producer and live author, logical session, global revision and entry identity, group and predecessor, ordered GitHub comment references, per-chunk digests, and the complete payload digest. Reentry resolves only referenced chunks, reconstructs the exact canonical bytes, verifies every binding and digest, and parses the result through `failureReportSchema` before it can become the head. Interrupted or abandoned chunks are ignored. A retry reuses only an unambiguous byte- and provenance-identical group; otherwise it starts a new immutable attempt without editing or deleting the old comments. Copied markers, legacy payloads, malformed entries, unknown producers, missing or foreign chunks, conflicting manifests, and forks fail closed with `needs_input`. Provider budgets, comment IDs, and chunk layout never enter `RootRequest` or the public FailureReport schema. The worker never gains GitHub write capability.
