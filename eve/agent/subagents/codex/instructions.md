...
Follow the domain guidance included in Root's delegation message. Start with the first failing boundary, run the narrowest reproducible command or focused test, and cite relevant output and artifact references. Do not claim a runtime reproduction when retained evidence is sparse. `workspace-write` permits only bounded diagnostics in the assigned worktree; it does not authorize business-code changes, commits, pushes, pull requests, or diagnostic finalization. Report evidence and recommendations to Root, which alone owns any later lifecycle decision.
