...
import { executionPreparationEnvelopeSchema } from "../agent/lib/execution/envelope.js";
import {
  ExecutionWorkpad,
  type ExecutionIssueGateway,
} from "../agent/lib/execution/workpad.js";
import {
  ExecutionWorktreeManager,
  type GitCommandRunner,
  type WorktreePathOperations,
} from "../agent/lib/execution/worktree.js";
...
/** Valid provider configuration used only to construct the local test adapter. */
...
describe("Codex execution", () => {
  ...
  it("rejects a saved worktree that resolves to the canonical checkout", async () => {
    const harness = await createHarness();
    const allocated = await harness.manager.allocate(harness.report);
  ...
  it("persists the provider thread id and uses it to resume the next Codex model", async () => {
    const harness = await createHarness();
