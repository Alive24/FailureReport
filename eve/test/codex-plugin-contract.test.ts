...
const mcpBundleFile = new URL("mcp/server.mjs", pluginRoot);
...
describe("FailureReport Codex plugin contract", () => {
  it("packages a directly loadable, plugin-local MCP server map", async () => {
    ...
    const bundle = await readFile(mcpBundleFile, "utf8");
    expect(bundle).toMatch(/^\/\/ Generated by pnpm build:codex-plugin/iu);
    expect(bundle.length).toBeGreaterThan(100_000);
    expect(bundle).not.toMatch(/\/Users\/|\.codex\/plugins\/cache/iu);
  });
