...
To use the public Root MCP surface through Codex, load the repository-local plugin at `packages/codex-plugin/failure-report`. Its `.mcp.json` starts the external `@failure-report/mcp-adapter` wrapper, which exposes the single `failure_report` tool and supervises runtime readiness before it calls the default Eve Channel.
